...
public class WithDrawInterfaceTest extends PlanetAutoConfigTestBase {
    ...
    public void withDrawTest() {
        ...
        withDrawRequest.getReqContext().setAccountName(testAccount.getName());
        withDrawRequest.setAmount(BigInteger.ZERO);
        // set other params...

        // 3. 执行请求
        WithDrawResponse withDrawResponse = planetBAO.withDraw(withDrawRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(withDrawRequest);
        TransactionModel transaction = collectTransaction(withDrawRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                withDrawResponse.getRespContext().getErrorMsg(),
                withDrawResponse.getRespContext().isSuccess());
    }
}
