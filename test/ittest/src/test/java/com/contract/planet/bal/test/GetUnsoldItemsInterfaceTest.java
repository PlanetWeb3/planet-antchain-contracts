...
public class GetUnsoldItemsInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void getUnsoldItemsTest() {
        ...
        // 2.2. 构造request
        GetUnsoldItemsRequest getUnsoldItemsRequest = new GetUnsoldItemsRequest();
        getUnsoldItemsRequest.getReqContext().setAccountName(testAccount.getName());
        // set other params...

        // 3. 执行请求
        GetUnsoldItemsResponse getUnsoldItemsResponse =
                planetBAO.getUnsoldItems(getUnsoldItemsRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(getUnsoldItemsRequest);
        TransactionModel transaction = collectTransaction(getUnsoldItemsRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                getUnsoldItemsResponse.getRespContext().getErrorMsg(),
                getUnsoldItemsResponse.getRespContext().isSuccess());
    }
}
