...
public class ChangeKInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void changeKTest() {
        ...
        // 2.2. 构造request
        ChangeKRequest changeKRequest = new ChangeKRequest();
        changeKRequest.getReqContext().setAccountName(testAccount.getName());
        changeKRequest.setK(BigInteger.ZERO);
        // set other params...

        // 3. 执行请求
        ChangeKResponse changeKResponse = planetBAO.changeK(changeKRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(changeKRequest);
        TransactionModel transaction = collectTransaction(changeKRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                changeKResponse.getRespContext().getErrorMsg(),
                changeKResponse.getRespContext().isSuccess());
    }
}
