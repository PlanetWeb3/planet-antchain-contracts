...
public class GetOwnerInterfaceTest extends PlanetAutoConfigTestBase {
    ...
    public void getOwnerTest() {
        ...
        getOwnerRequest.getReqContext().setAccountName(testAccount.getName());
        // set other params...

        // 3. 执行请求
        GetOwnerResponse getOwnerResponse = planetBAO.getOwner(getOwnerRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(getOwnerRequest);
        TransactionModel transaction = collectTransaction(getOwnerRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                getOwnerResponse.getRespContext().getErrorMsg(),
                getOwnerResponse.getRespContext().isSuccess());
    }
}
