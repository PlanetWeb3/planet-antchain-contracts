...
public class RedeemItemsInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void redeemItemsTest() {
        ...
        // 2.2. 构造request
        RedeemItemsRequest redeemItemsRequest = new RedeemItemsRequest();
        redeemItemsRequest.getReqContext().setAccountName(testAccount.getName());
        redeemItemsRequest.setItemId(BigInteger.ZERO);
        redeemItemsRequest.setAmount(BigInteger.ZERO);
        // set other params...

        // 3. 执行请求
        RedeemItemsResponse redeemItemsResponse = planetBAO.redeemItems(redeemItemsRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(redeemItemsRequest);
        TransactionModel transaction = collectTransaction(redeemItemsRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                redeemItemsResponse.getRespContext().getErrorMsg(),
                redeemItemsResponse.getRespContext().isSuccess());
    }
}
