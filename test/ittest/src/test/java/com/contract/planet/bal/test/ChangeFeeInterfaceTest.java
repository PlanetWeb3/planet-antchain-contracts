...
public class ChangeFeeInterfaceTest extends PlanetAutoConfigTestBase {
    ...
    public void changeFeeTest() {
        ...
        // 2.2. 构造request
        ChangeFeeRequest changeFeeRequest = new ChangeFeeRequest();
        changeFeeRequest.getReqContext().setAccountName(testAccount.getName());
        changeFeeRequest.setFee(BigInteger.ZERO);
        // set other params...

        // 3. 执行请求
        ChangeFeeResponse changeFeeResponse = planetBAO.changeFee(changeFeeRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(changeFeeRequest);
        TransactionModel transaction = collectTransaction(changeFeeRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                changeFeeResponse.getRespContext().getErrorMsg(),
                changeFeeResponse.getRespContext().isSuccess());
    }
}
