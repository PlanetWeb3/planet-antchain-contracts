...
public class GetContractBalanceInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void getContractBalanceTest() {
        ...
        // 2.2. 构造request
        GetContractBalanceRequest getContractBalanceRequest = new GetContractBalanceRequest();
        getContractBalanceRequest.getReqContext().setAccountName(testAccount.getName());
        // set other params...

        // 3. 执行请求
        GetContractBalanceResponse getContractBalanceResponse =
                planetBAO.getContractBalance(getContractBalanceRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(getContractBalanceRequest);
        TransactionModel transaction = collectTransaction(getContractBalanceRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                getContractBalanceResponse.getRespContext().getErrorMsg(),
                getContractBalanceResponse.getRespContext().isSuccess());
    }
}
