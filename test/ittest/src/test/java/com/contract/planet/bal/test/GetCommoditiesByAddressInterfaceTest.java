...
public class GetCommoditiesByAddressInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void getCommoditiesByAddressTest() {
        ...
        // 2.2. 构造request
        GetCommoditiesByAddressRequest getCommoditiesByAddressRequest =
                new GetCommoditiesByAddressRequest();
        getCommoditiesByAddressRequest.getReqContext().setAccountName(testAccount.getName());
        getCommoditiesByAddressRequest.setOwner("");
        // set other params...

        // 3. 执行请求
        GetCommoditiesByAddressResponse getCommoditiesByAddressResponse =
                planetBAO.getCommoditiesByAddress(getCommoditiesByAddressRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(getCommoditiesByAddressRequest);
        TransactionModel transaction = collectTransaction(getCommoditiesByAddressRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                getCommoditiesByAddressResponse.getRespContext().getErrorMsg(),
                getCommoditiesByAddressResponse.getRespContext().isSuccess());
    }
}
