...
public class GetPlanetinumInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void getPlanetinumTest() {
        ...
        // 2.2. 构造request
        GetPlanetinumRequest getPlanetinumRequest = new GetPlanetinumRequest();
        getPlanetinumRequest.getReqContext().setAccountName(testAccount.getName());
        getPlanetinumRequest.setOwner("");
        // set other params...

        // 3. 执行请求
        GetPlanetinumResponse getPlanetinumResponse = planetBAO.getPlanetinum(getPlanetinumRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(getPlanetinumRequest);
        TransactionModel transaction = collectTransaction(getPlanetinumRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                getPlanetinumResponse.getRespContext().getErrorMsg(),
                getPlanetinumResponse.getRespContext().isSuccess());
    }
}
