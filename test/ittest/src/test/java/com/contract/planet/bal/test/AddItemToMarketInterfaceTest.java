...
public class AddItemToMarketInterfaceTest extends PlanetAutoConfigTestBase {

    @Test
    public void addItemToMarketTest() {
        ...
        // 2.2. 构造request
        AddItemToMarketRequest addItemToMarketRequest = new AddItemToMarketRequest();
        addItemToMarketRequest.getReqContext().setAccountName(testAccount.getName());
        addItemToMarketRequest.setTokenId(BigInteger.ZERO);
        addItemToMarketRequest.setAmount(BigInteger.ZERO);
        addItemToMarketRequest.setPrice(BigInteger.ZERO);
        // set other params...

        // 3. 执行请求
        AddItemToMarketResponse addItemToMarketResponse =
                planetBAO.addItemToMarket(addItemToMarketRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(addItemToMarketRequest);
        TransactionModel transaction = collectTransaction(addItemToMarketRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                addItemToMarketResponse.getRespContext().getErrorMsg(),
                addItemToMarketResponse.getRespContext().isSuccess());
    }
}
