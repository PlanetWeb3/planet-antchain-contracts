...
public class GetBalanceInterfaceTest extends PlanetAutoConfigTestBase {
    ...
    public void getBalanceTest() {
        ...
        getBalanceRequest.getReqContext().setAccountName(testAccount.getName());
        // set other params...

        // 3. 执行请求
        GetBalanceResponse getBalanceResponse = planetBAO.getBalance(getBalanceRequest);

        // 4. 收集请求执行的区块链交易信息
        // 4.1. 查看请求关联的交易、收据
        ReceiptModel receipt = collectReceipt(getBalanceRequest);
        TransactionModel transaction = collectTransaction(getBalanceRequest);
        ...
        // 5. 检查请求是否成功
        assertTrue(
                getBalanceResponse.getRespContext().getErrorMsg(),
                getBalanceResponse.getRespContext().isSuccess());
    }
}
